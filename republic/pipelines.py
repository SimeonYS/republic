...
class RepublicPipeline:

    # Database setup
    conn = sqlite3.connect('republic.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `republic`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM republic WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO republic (date, title, link, content)"
